...
4. **Power Automate akışı** — **kurulmayacak (karar verildi).** Günlük cron (`17 3 * * *` UTC, Türkiye saatiyle 06:17) her gece build'i çalıştırıp SharePoint içeriğini yeniden çekiyor; bu yeterli görüldü. Beklemeden yayınlamak gerekirse GitHub Actions'daki **Run workflow** (`workflow_dispatch`) düğmesi kullanılır. Fikir değişirse workflow'daki `repository_dispatch` tetikleyicisi zaten bağlı — yalnızca akışın kurulması gerekir.
...
6b. ~~**DNS geçişi**~~ — Site canlı: **https://www.sonopto.com**. DNS Metunic'te (`ns1/ns2.metunic.com.tr`). `www` CNAME → SWA varsayılan adresi; her iki alan adı da Azure'da `Ready` (TXT ile doğrulandı, DigiCert sertifikası DNS değişmeden önce hazırlandı, bu yüzden kesinti olmadı). Apex `sonopto.com`, Metunic'in URL yönlendirmesiyle `https://www.sonopto.com`'a **301** atıyor; A kaydı Metunic'in yönlendirme sunucusunda (`46.20.6.99`), Odoo'nun eski IP'si kaldırıldı.
   - **Bilinen boşluk:** `https://sonopto.com` sertifika hatası veriyor — Metunic'in yönlendirmesi yalnızca `http://` kaynağını karşılıyor ve o alan adı için sertifika sunmuyor. Kapatmanın yolu DNS'i Azure DNS'e taşımak (apex'te ALIAS gerekiyor; Metunic ALIAS desteklemiyor, M365'in DNS'i de desteklemiyor). Sabit A kaydı **çözüm değil**: SWA'nın IP'si Traffic Manager üzerinden dağıtılıyor ve bir saat içinde `51.124.12.35` → `20.50.153.39` değiştiği gözlendi.
   - Geri dönüş: `www` CNAME'ini `sonopto.odoo.com`'a çevirmek yeterli; TTL'ler 300s.
7. ~~**Marka kimliği**~~ — Sonopto Kurumsal Kimlik Kılavuzu'ndan (v1.0, "Sonopto" design system artifact'i) alınan gerçek değerlerle uygulandı: renkler (`#000000`/`#ffffff`/Neon Yeşil `#39ff14`), tipografi (Alata/Roboto Slab/Roboto Mono, self-hosted `@fontsource`), radius-none kuralı ve gerçek logo/favicon/OG görseli (`public/logo.png`, `public/favicon.svg`, `public/og-default.png`). Gri rampası kapatıldı: kılavuz üç gri veriyor ama bunları ölçek adımı olarak değil rol olarak tanımlıyor. Yalnızca Açık Gri `#F5F5F5` bir adıma temiz oturuyor ve `neutral-50` olarak uygulandı. Orta Gri `#6B6B6B` ve Koyu Gri `#2A2A2A` bilinçli olarak alınmadı — gerekçeleri `global.css`'teki yorumda kontrast sayılarıyla yazılı. Ayrıca beyaz zeminde AA'yı geçemeyen (3.45:1) dokuz `text-neutral-400` kullanımı (form "(zorunlu)" etiketleri ve SSS ikonu) `neutral-500`'e taşındı; siyah zemindekiler 6.08:1 ile sorunsuz olduğu için korundu. Wordmark artık gerçek fontuyla (**Visby Heavy**) render ediliyor: lisanslı TTF repoya girmiyor (`.gitignore`'da), yalnızca `sonopto.` için gereken glifleri içeren 1 KB'lık alt küme `public/fonts/visby-heavy-logo.woff2` olarak yayınlanıyor. Alt kümeyi yeniden üretmek için: `pyftsubset "Visby Heavy.ttf" --text="sonoptoSONOPTO." --flavor=woff2 --layout-features="" --no-hinting --desubroutinize --output-file=public/fonts/visby-heavy-logo.woff2`.
8. **İçerik incelemesi** — Vizyon & Misyon ve Hakkımızda sayfaları artık kılavuzdaki gerçek marka cümleleriyle güncellendi ("Sonopto kurduysa sorun çıkmaz.", "Müşterinin krizi, bizim önceliğimizdir.", "Sahadaki Çözüm Ortağınız"). Hizmet detayları ve SSS metinleri hâlâ taslak; yayına almadan önce Sonopto ekibi tarafından gözden geçirilmeli. `Projects` listesi dolduruldu — KOSGEB, Fuar Ekranları ve Konferans Salonu yayında, kapak görselleri `ProjectMedia`'dan geliyor. Yeni projeler SharePoint'ten eklenince günlük cron (03:17 UTC) veya `repository_dispatch` ile yayına çıkar.
