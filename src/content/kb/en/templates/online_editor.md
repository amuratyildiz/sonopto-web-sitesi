...
-   Click on **Playlists**
-   Open a playlist
-   Click on a layer where you will add a template
-   If the layer has no media, the apps dialog will open. Otherwise, click on **Add New**
-   Click on **Add from Templates**
-   Click on a template category
-   Click on a template
...
_Transparency_: control the transparency of the selected object.

_Shadow_: click on shadow color to open color chooser. Control the shadow size using the seek bar.
...
_Align the object to_: left, center vertically, center horizontally, right, top, bottom

_Send Backward_: send the object to the back by 1 step

_Bring forward_: bring the object to the front by 1 step

_Send to Back_: send the object behind all other objects

_Bring to Front_: bring the object on top of all other objects

Change the left and top position of the object

_Objects can be moved by dragging and droppping them to another position. Or by using the arrow keys on the keyboard._
...
_Delete_: Delete the selected object. Objects can also be deleted by pressing delete key on the keyboard

_Snap to Grid_: If checked, move the objects on a grid instead of freely moving them

_Zoom_: Put the cursor on a zone in the tempate and zoom in and out the template.
...
-   Click on delete on the right to delete an item
...
-   Click on an image from the list to add it to the template
...
-   Resize it and move it in the template to fit
...
-   Pixabay is a third-party searchable image library

#### Shapes

-   Choose through different categories of shapes
-   Shapes can be customized in size and color

#### Icons

-   Search through transparent vector images
...
-   The lock near each layer allows you to lock a layer from being modified, resized or moved
-   The eye is used to hide/show the layer

* * *

Once the template design is ready, click on **Save**.

The template will be processed and saved as an image.

To edit it again, click on **Open Editor**.
